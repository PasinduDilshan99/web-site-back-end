...
public class TourQueries {
    ...
    public static final String GET_POPULAR_TOURS = """
            ...
            LEFT JOIN common_status cs_img ON ti.status_id = cs_img.id
            LEFT JOIN tour_destination td ON t.tour_id = td.tour_id
            INNER JOIN review r ON t.tour_id = r.tour_id
            LEFT JOIN common_status rs ON r.status_id = rs.id
            GROUP BY t.tour_id, ti.id, r.review_id
            ORDER BY t.tour_id, ti.id, r.review_id
            """;
